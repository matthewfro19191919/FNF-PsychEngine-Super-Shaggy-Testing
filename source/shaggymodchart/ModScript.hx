...
class ModScript {
	...
	public static function modInit(Mod:Int):Void {
		...
		for (i in 0...8) {
			...
			tweenStart([obj_game, "strums." + i + ".lines.0"], {
				a: 270.,
				ax: 0.,
				ay: 0.,
				w: 1.
			}, 0.1, "inout_quad");
			tweenStart([obj_game, "strums." + i + ".lines.1"], {
				a: 270.,
				ax: 0.,
				ay: 0.,
				w: 1.
		...
		if (started) {
			if (mod == 0) {
				...
				for (i in 0...8) {
					...
					if (i < 4) {
						an = an + MathUtil.degreesToRadians(180);
					} // Assumes MathUtil exists
	...
	public static function onBeatHit(beat:Int):Void {
		...
		class Sys {
			public static var skin = {sep: 160.0};
			public static var options = {downscroll: false};
		}
		...
		class Engine {
			...
			static function get_obj_game():Dynamic {
				return obj_game;
			} // Access global obj_game
